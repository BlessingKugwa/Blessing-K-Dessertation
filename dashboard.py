...
st.set_page_config(
    page_title="Operational Risk Management System",
    page_icon="🛡️",
    layout="wide",
...
CORE_FEATURES = [
    "FREQUENCY", "SEVERITY", "OPVAR(%)",  "GDP (%)", "VIX",
...
def load_models():
    ...
    scored_df = pd.read_csv('outputs/scores.csv')
...
with col1:
    st.markdown('<h1 class="hero-title">Operational Risk Management System</h1>', unsafe_allow_html=True)
    ...
    **🎯 Core Capabilities:**
    ...
    - Predict loss amounts with Random Forest Regression 
    - Classify risk levels (LowHigh) with balanced RF Classifier
...
with col2:
    ...
    <div style="background: linear-gradient(135deg, #3498db, #2980b9); color: white; padding: 1.5rem; border-radius: 15px; margin-bottom: 2rem; text-align: center;">
        ...
        <ul style="text-align: left; font-size: 0.9rem;">
            <li>✅ R²: **0.82** (Loss Prediction)</li>
            <li>✅ F1-macro: **0.75** (Risk Class, tuned)</li>
            <li>✅ **5.9%** Anomaly Detection Rate</li>
...
with kpi_container:
    ...
    with kpi_col5:
        st.markdown("""
        <div class="metric-card" style="height: 120px; display: flex; flex-direction: column; justify-content: center;">
            <h3 style="margin: 0 0 0.5rem 0;">🎯 F1-Score (tuned)</h3>
            <h1 style="color: #3498db; font-size: 2rem; margin: 0;">0.75</h1>
        </div>
        """, unsafe_allow_html=True)
...
with tab1:
    ...
    # Timeline chart (NEW) - Single line total RISK_SCORE trend
    fig_timeline = px.line(
        filtered_df.sort_values('DATE'), 
        x='DATE', y='RISK_SCORE',
        title="Overall Risk Score Evolution Over Time (2014-2025)",
        hover_data=['EVENT TYPE', 'ALERT_TIER']
    )
    fig_timeline.update_layout(
        ...
        yaxis=dict(title="Total RISK_SCORE")
    )
    ...
    # Average Loss Amount by Process Area Bar
    st.markdown("### 💰 Average Loss Amount by Process Area")
    avg_loss_process = scored_df.groupby('PROCESS AREA')['LOSS AMOUNT(USD)'].mean().sort_values(ascending=False).reset_index()
    fig_loss_process = px.bar(avg_loss_process.head(10), x='PROCESS AREA', y='LOSS AMOUNT(USD)',
                              title="Avg Loss $ per Process Area (Top 10)",
                              color='LOSS AMOUNT(USD)', color_continuous_scale='Viridis')
    fig_loss_process.update_layout(height=450, xaxis_tickangle=45)
    st.plotly_chart(fig_loss_process, use_container_width=True)
    


    st.markdown("### 🚨 Anomaly Counts by Event Type & Process Area")
    anomaly_counts_table = scored_df.groupby(['EVENT TYPE', 'PROCESS AREA'])['ANOMALY_COMBINED'].sum().reset_index()
    anomaly_counts_table.columns = ['Event Type', 'Process Area', 'Anomaly Count']
    anomaly_counts_table = anomaly_counts_table.sort_values('Anomaly Count', ascending=False)
    st.dataframe(anomaly_counts_table, use_container_width=True)
    
    # Heatmap with exact counts
    anomaly_counts_pivot = scored_df.pivot_table(values='ANOMALY_COMBINED', index='EVENT TYPE', columns='PROCESS AREA', aggfunc='sum', fill_value=0)
    fig_anom_heatmap = px.imshow(anomaly_counts_pivot, title="Anomaly Counts Heatmap (Exact Numbers)", 
                                 color_continuous_scale='Reds', text_auto=True)
    fig_anom_heatmap.update_layout(height=500)
    st.plotly_chart(fig_anom_heatmap, use_container_width=True)
...
with tab3:
    ...
    with col1:
        ...
        feat_imp_loss = {
            'LOSS_PER_FREQ': 0.48, 'SCENARIO_ENC': 0.17,  'OPVAR(%)': 0.12,
             'RISK_COMPOSITE': 0.11, 'SEV_OPVAR_RATIO': 0.03, 'GDP_VIX_RATIO': 0.03, 'VIX': 0.02,
            'EVENT_ENC': 0.01,  'DAY_OF_WEEK': 0.01,
            'GDP (%)': 0.01, 
        }
    ...
    with col2:
        ...
        # Tuned Confusion Matrix from model.py (binary Low/High, F1-macro ~0.75)
        cm_data = [[387, 18], [9, 16]]  # [[TN, FP], [FN, TP]]
        fig_cm = px.imshow(cm_data, 
                         labels=dict(x="Predicted", y="Actual", color="Count"),
                         x=['Low', 'High'], 
                         y=['Low', 'High'],
                         title="Risk Classification Performance (Tuned F1-macro: 0.75)",
                         color_continuous_scale='Blues')
        st.plotly_chart(fig_cm, width='stretch')
    
    col_anom1, col_anom2 = st.columns(2)
    with col_anom1:
        # Anomaly Distribution by Event Type
        st.markdown("#### 📊 Anomalies by Event Type")
        anomaly_by_event = scored_df.groupby('EVENT TYPE')['ANOMALY_COMBINED'].agg(['count', 'sum']).reset_index()
        anomaly_by_event['anomaly_rate'] = anomaly_by_event['sum'] / anomaly_by_event['count'] * 100
        anomaly_by_event = anomaly_by_event.sort_values('sum', ascending=False)
        fig_anom_event = px.bar(anomaly_by_event.head(10), x='EVENT TYPE', y='sum', 
                               title="Event Types (Top 10)",
                               hover_data=['anomaly_rate'],
                               color='sum', color_continuous_scale='Reds')
        fig_anom_event.update_layout(height=400, xaxis_tickangle=45)
        st.plotly_chart(fig_anom_event, use_container_width=True)
    
    with col_anom2:
        # Anomaly Distribution by Process Area
        st.markdown("#### 🏭 Anomalies by Process Area")
        anomaly_by_process = scored_df.groupby('PROCESS AREA')['ANOMALY_COMBINED'].agg(['count', 'sum']).reset_index()
        anomaly_by_process['anomaly_rate'] = anomaly_by_process['sum'] / anomaly_by_process['count'] * 100
        anomaly_by_process = anomaly_by_process.sort_values('sum', ascending=False)
        fig_anom_process = px.bar(anomaly_by_process.head(10), x='PROCESS AREA', y='sum', 
                                 title="Process Areas (Top 10)",
                                 hover_data=['anomaly_rate'],
                                 color='sum', color_continuous_scale='Oranges')
        fig_anom_process.update_layout(height=400, xaxis_tickangle=45)
        st.plotly_chart(fig_anom_process, use_container_width=True)
    
    # Risk Classification Feature Importance (matching Loss Prediction style)
    st.markdown("#### 🎯 Risk Classification - Top Features")
    feat_imp_risk = {
        'LOSS_PER_FREQ': 0.28, 'GDP_VIX_RATIO': 0.15, 'RISK_COMPOSITE': 0.13,  'OPVAR(%)': 0.12, 'SEV_OPVAR_RATIO': 0.08, 'GDP (%)': 0.07,
        
         'VIX': 0.05,'DAY_OF_WEEK': 0.03, 'PROCESS_ENC': 0.03, 'EVENT_ENC': 0.02,
    }
    fig_risk_imp = px.bar(y=list(feat_imp_risk.values()), x=list(feat_imp_risk.keys()), 
                          title="Top Features Driving Risk Classification")
    st.plotly_chart(fig_risk_imp, width='stretch')
    
    st.markdown("#### 📈 Model Summary")
    st.markdown("""
    | Model | Purpose | Key Metric | Performance |
    |-------|---------|------------|-------------|
    | **Isolation Forest** | Anomaly Detection | Contamination | 5.9% |
    | **RF Regression** | Loss Prediction | R² Score | **0.82** |
    | **RF Classifier** | Risk Tier (Low/High) | F1-macro (tuned) | **0.75** |
